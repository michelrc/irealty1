 ...
 */
class MessageWidget extends CJuiTabs
{
    ...
    public $headerTemplate = '<li><a href="{url}" class="langtab" rel="{lang}" title="{id}">{title}</a></li>';
    /**
    ...
    public function run()
    {
        ...
        foreach (Language::getLanguages() as $lang) {
            ...
            $item = I18nMessage::model()->findByPk(array('id' => $this->model->id, 'language' => $lang->id));
            ...
            $this->tabs[$lang->id] = array(
                ...
                'content' => strtr($this->_template, array(
                    ...
                    '{error}' => $item ? CHtml::error($item, $this->attribute) : "",
                )),
                'lang' => $lang->id
            );
        }
        ...
        foreach ($this->tabs as $title => $content) {
            ...
            if (!is_array($content)) {
                ...
                $tabsOut .= strtr($this->headerTemplate, array('{title}' => $title, '{lang}' => $content['lang'], '{url}' => '#' . $tabId)) . "\n";
        ...
        Yii::app()->getClientScript()->registerScript(__CLASS__ . '#' . $id, "jQuery('#{$id}').tabs($options);jQuery('.ui-tabs').tabs({
            select: function(event, ui) {
                if (!window.selectionBlocked){ // Selected by the user
                    window.selectionBlocked = true;
                    jQuery('.ui-tabs').not(this).tabs('option', 'selected', ui.index);
