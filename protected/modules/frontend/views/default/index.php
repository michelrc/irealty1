    ...
    <div id="main-carousel">
        <?php foreach ($banner_slide as $banner): ?>
            <div class="carousel-item">
                <img src="<?php echo $banner['image'] ?>" alt="<?php echo $banner['image_alt'] ?>"
            ...
            <span class="span-text">
                ...
                <div data-toggle="buttons" class="btns btn-group">
                    <label for="to-buy" class="btn active hover-transition">
                        <input type="radio" value="buy" name="cat" onchange="change_url_searcher_form('buy')" checked
                               id="buy">
                    ...
                    <label for="to-rent" class="btn hover-transition">
                        <input type="radio" name="cat" id="rent" value="rent"
                               onchange="change_url_searcher_form('rent')">
                    ...
                    <div class="input-group">
                        <input id="searcher_input" name="q" type="text"
                               ...
                               else: ?>Search by address, city, zip code<?php endif ?>"
                               class="form-control input-lg">
                    <span class="input-group-btn">
            <button type="submit" id="search_form" class="btn btn-primary btn-lg hover-transition"><i class="fa fa-search"></i>
            </button></span>
                    </div>
                </div>
                ...
                <?php foreach ($leading_properties as $property): ?>
                    <div class="property-item">
                        <?php if (isset($property['image'])): ?>
                            <img src="<?php echo $property['image'] ?>" alt="<?php echo $property['image_alt'] ?>"
                                 class="image-responsive">
                        <?php else: ?>
                            ...
                            <div class="details">
                                <ul class="list-unstyled">
                                    ...
                                    <li><i class="fa fa-arrows"></i><span
                                            class="detail-title"><?php echo Yii::t('Property', 'Area') ?></span><span><?php echo $property['area'] ?>
                                            m<sup>2</sup></span>
                                    </li>
                                ...
                                <a href="<?php echo $this->createUrl('/frontend/default/property', array('id' => $property['id'])) ?>"
                                   class="text-right rounded-btn">
                                    <?php echo Yii::t('Property', 'View') ?>
                                    <br><strong><?php echo Yii::t('Property', 'Details') ?></strong>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-title">
                        ...
                        <div data-toggle="buttons" class="blue-filters btn-group pull-right">
                            <label for="buy" class="btn active">
                                <input type="radio" name="options_category" value="buy" onchange="filter_properties()"
                                       checked id="buy">
                            ...
                            <label for="rent" class="btn">
                                <input type="radio" name="options_category" value="rent" onchange="filter_properties()"
                                       id="rent">
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                <div data-toggle="buttons" class="category-filters btn-group pull-left">
                    <label for="all"
                           class="btn <?php if ($type_selected == 'all'): ?>active<?php endif ?> hover-transition">
                        <input type="radio" value="all" name="options_type" onchange="filter_properties()" checked
                               id="all"><?php echo Yii::t('labels', 'All') ?>
                    </label>
                    <?php foreach ($property_types as $type): ?>
                        <label for="house" class="btn hover-transition">
                            <input type="radio" name="options_type" onchange="filter_properties()"
                                   value="<?php echo $type['id'] ?>" id=""> <?php echo $type['type'] ?>
                        </label>
                    <?php endforeach ?>
                </div>
            </div>
        ...
        <div class="row notify-section">
            <div class="col-lg-5 col-md-5 col-sm-6">
                ...
                <div class="input-group">
                    <input title="<?php echo t('The email is not valid') ?>" id="newsletter_input" type="text"
                           placeholder="<?php if (isset($newsletter_placeholder)):echo $newsletter_placeholder;
                           else: ?>Get noticed of new properties added<?php endif ?>"
                           class="form-control input-lg"><span class="input-group-btn">
            <button onclick="add_subscribers()" class="btn btn-primary btn-lg hover-transition"><i
                    class="fa fa-angle-double-right fa-inverse"></i>
            </button></span>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-3 col-sm-4 text-right">
                <?php if ($show_more_properties > 6): ?>
                    <a href="<?php echo $this->createUrl('/frontend/default/buy_list') ?>" class="view-more"
                       id="buy_view_more">
                    ...
                    <a href="<?php echo $this->createUrl('/frontend/default/rent_list') ?>" hidden="hidden"
                       class="view-more" id="rent_view_more">
                        <?php if (isset($recent_property_view_more)):
                            echo $recent_property_view_more;
                        else:?>
                            View more properties
            ...
            <?php foreach ($why_shop_icons as $wshop_icons): ?>
                <div class="promo-item text-center">
                    <div class="icon-wrapper">
                        <img src="<?php echo $wshop_icons['icon_image'] ?>"
                             alt="<?php echo $wshop_icons['icon_image_alt'] ?>"/>
                ...
                <div class="news-item">
                    <header class="news-header">
                        <div class="date-badge">
                            <div class="rounded-wrapper"><span
                                    class="month text-uppercase"><?php echo $b['date_month'] ?></span><span
                                    class="day"><?php echo $b['date_day'] ?></span>
                            </div>
                        </div>
                        <div class="news-title">
                            <a href="<?php echo $this->createUrl('/frontend/default/blog', array('id' => $b['id'])) ?>"
                               class="hover-transition">
                                <h2><?php echo $b['title'] ?></h2></a>
                            <span
                                class="small"><?php echo Yii::t('Home', 'Posted by') ?><?php echo $b['author'] ?></span>
                        </div>
                    </header>
            ...
            <div id="testimonials-nav">
                <?php foreach ($testimonials as $key => $t): ?>
                    <div class="nav-item <?php if ($key == 0): ?>active<?php endif ?>">
                        <div class="marker"><img src="<?php echo app()->baseUrl; ?>/static/imgs/arrow-down.png">
                        </div>
                        <div class="nav-image">
                            <?php if (isset($t['photo'])): ?>
                                <img src="<?php echo $t['photo'] ?>" alt="<?php echo $t['photo_alt'] ?>"
                                     class="image-responsive">
                            <?php else: ?>
                        ...
                        <div class="nav-caption text-center">
                            <h4><?php echo $t['author_name'] ?><span
                                    class="small"><?php echo $t['company_name'] ?></span>
                            </h4>
                        </div>
                    </div>
                <?php endforeach ?>
